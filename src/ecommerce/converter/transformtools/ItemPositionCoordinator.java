...
public class ItemPositionCoordinator {
    ...
    public static String[][] storeRelevantPositions(String[][] daten_original, int columns, String[] relevanteItems) {
        ...
        String[][] items_positions = new String[relevanteItems.length][2];
        // Positionen der Daten so lange wie Anzahl relevante Daten (Reihen) und 2 Spalten

        for (int anzahl_relevante_items=0; anzahl_relevante_items<relevanteItems.length; anzahl_relevante_items++) {

            String relevante_information = relevanteItems[anzahl_relevante_items];

            for (int i = 0; i < columns; i++) {
                if (daten_original[0][i].equals(relevante_information)) {
